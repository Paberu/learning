from LinkedWithDummy import Node, DummyNode, LinkedList2
import unittest
...
class TestLinkedList(unittest.TestCase):
    ...
    def test_delete1(self):
        ...
        self.assertEqual(self.list1.head.next, self.n_first)
        self.assertEqual(self.list1.tail.prev, self.n_last)

        self.list1.delete(12)
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list1), [55, 128, 12, 56, 128])
        self.assertNotEqual(self.list1.head.next, self.n_first)
        self.assertEqual(self.list1.tail.prev, self.n_last)

        self.list1.delete(128, all=True)
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list1), [55, 12, 56])
        self.assertNotEqual(self.list1.head.next, self.n_first)
        self.assertNotEqual(self.list1.tail.prev, self.n_last)

    def test_delete2(self):
        self.list2.delete(12, all=True)
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list2), [])
        self.assertEqual(self.list2.head.next, self.list2.tail)
        self.assertEqual(self.list2.tail.prev, self.list2.head)

    def test_delete3(self):
        self.list4.delete(12, all=True)
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list4), [])
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list2), [])
        self.assertEqual(self.list4.head.next, self.list4.tail)
        self.assertEqual(self.list4.tail.prev, self.list4.head)

    def test_delete4(self):
        self.list5.delete(12)
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list5), [])
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list2), [])
        self.assertEqual(self.list5.head.next, self.list5.tail)
        self.assertEqual(self.list5.tail.prev, self.list5.head)

    def test_delete5(self):
        self.n_first = self.list6.head.next
        self.n_last = self.list6.tail.prev
        self.list6.delete(12)
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list6), [10, 11, 13, 14])
        self.assertEqual(self.list6.head.next, self.n_first)
        self.assertEqual(self.list6.tail.prev, self.n_last)

        self.list6.delete(14)
        self.assertEqual(get_values_from_nodes_array_from_linked_list(self.list6), [10, 11, 13])
        self.assertEqual(self.list6.head.next, self.n_first)
        self.assertNotEqual(self.list6.tail.prev, self.n_last)

    def test_insert1(self):
        self.n_first = self.list1.head.next
        self.n_last = self.list1.tail.prev
    ...
    def test_insert4(self):
        ...
        self.assertEqual(self.list2.head.next, node)
        self.assertEqual(self.list2.tail.prev, node)

    def test_clean(self):
        self.list1.clean()
        self.assertEqual(self.list1.head.next, self.list1.tail)
        self.assertEqual(self.list1.tail.prev, self.list1.head)
    ...
    def test_add_in_head(self):
        ...
        self.assertEqual(self.list1.head.next, node)
        self.assertNotEqual(self.list1.head.next, self.n_first)
        self.assertEqual(self.list1.tail.prev, self.n_last)
...
def get_values_from_nodes_array_from_linked_list(list_to_values):
    values = []
    node = list_to_values.head.next
    for _ in range(list_to_values.len()):

...
class TestNativeDictionary(unittest.TestCase):
    ...
    def test_put1(self):
        ...
        for every_str in str_list:
            self.assertNotEqual(self.dict1.put(every_str, every_str), -1)

    def test_put2(self):
        str_list = self.test_string2.split()
        for every_str in str_list:
            self.assertNotEqual(self.dict2.put(every_str, every_str), -1)
        self.assertNotEqual(self.dict2.put('Python', 'imlovingit'), -1)
